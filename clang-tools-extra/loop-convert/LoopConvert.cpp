    ...
    if (const RecordDecl *rdp =
      ...
      if (const RecordDecl *rdc =
        ...
        if (locrdc.find("invalid") == std::string::npos &&
          ...
          // locrd
          //  << endl;
          std::string loc = clang::Lexer::findNextToken(rdc->getBeginLoc(), Rewrite.getSourceMgr(), Rewrite.getLangOpts())->getLocation().printToString(*sm);
    ...
    if (const LabelStmt *lp =
      ...
      if (const RecordDecl *rd =
        ...
        if (locrd.find("invalid") == std::string::npos &&
          ...
          // locrd
          //  << endl;
          std::string loc = clang::Lexer::findNextToken(rd->getBeginLoc(), Rewrite.getSourceMgr(), Rewrite.getLangOpts())->getLocation().printToString(*sm);
					if(!renamedWhileResolving[loc]){
          Rewrite.InsertTextAfterToken(
              clang::Lexer::findNextToken(rd->getBeginLoc(),
                                          Rewrite.getSourceMgr(),
              ...
              locrd);
					...
					structToEmit[rd->getNameAsString() + locrd] = Rewrite.getRewrittenText(rd->getSourceRange()) + ";\n\n";
          structBuff << Rewrite.getRewrittenText(rd->getSourceRange())
                     << ";\n\n";
          // Rewrite.ReplaceText(rd->getBeginLoc(), "\n");
          // mark this record decl visited.
...
public:
      ...
      if (const FieldDecl *fd =
        ...
        if (fd->getType().getUnqualifiedType().getAsString().find("struct") ==
          ...
          std::string type = fd->getType().getUnqualifiedType().getAsString();
					std::string structName = fd->getType().getUnqualifiedType().getBaseTypeIdentifier()->getName().str();
          //cout << "type " << type << endl;
          // cout<<type<<endl;
          if (type.find('[') == std::string::npos) {
            ...
            // cout<<"res "<<res<<endl;
            if (res.compare("0") != 0) {
              auto loc = fd->getTypeSourceInfo()->getTypeLoc().getUnqualifiedLoc().getBeginLoc();
							loc = clang::Lexer::findNextToken(loc, Rewrite.getSourceMgr(), Rewrite.getLangOpts())->getLocation();
							//cout<<"var "<<loc.printToString(*sm)<<endl;
              if (std::find(writtenLocs.begin(), writtenLocs.end(), loc.printToString(*sm)) == writtenLocs.end()) {
      ...
      if (const VarDecl *vd = Result.Nodes.getNodeAs<clang::VarDecl>("child")) {
        ...
        if (vd->getType().getUnqualifiedType().getAsString().find("struct") ==
          ...
          std::string type = vd->getType().getUnqualifiedType().getAsString();
					std::string structName = vd->getType().getUnqualifiedType().getBaseTypeIdentifier()->getName().str();
          //cout << "type " << type << endl;
          // cout<<type<<endl;
          if (type.find('[') == std::string::npos) {
            // this logic works because the type is always returned with
            ...
            // modified.
            if (res.compare("0") != 0) {
              auto loc = vd->getTypeSourceInfo()->getTypeLoc().getUnqualifiedLoc().getBeginLoc();
							loc = clang::Lexer::findNextToken(loc, Rewrite.getSourceMgr(), Rewrite.getLangOpts())->getLocation();
							//cout<<"var "<<loc.printToString(*sm)<<endl;
							if (std::find(writtenLocs.begin(), writtenLocs.end(), loc.printToString(*sm)) == writtenLocs.end()) {
                Rewrite.ReplaceText(loc, res);
  ...
  std::string findRecInRec(std::string ls, std::string target,
    ...
    if (structdepths[ls] == 0) {
      ...
      //cout<<"xxxxxxxxxxxxxxxxxxxxxxxxxxx"<<ls<<" "<<structparenChilMap[ls]<<endl;
  ...
  std::string findRecInParent(std::string ls, std::string target,
    ...
    if (depths[ls] == 0) {
      // find in the target in the list of the label ls.
      for (auto elem : record[ls]) {
         //cout << "findrecinparentdepth0 elem: " << elem << " target: " << target <<"depth "<<structdepths[elem]<< endl;
        if (elem.find(target) != std::string::npos && structdepths[elem] == 1 &&
            structBounds[elem].begin <= stoi(celoc)) {
          return elem;
        }
      }
    ...
    for (auto elem : record[ls]) {
      //cout << "findrecinparent elem: " << elem << " target: " << target <<"depth "<<structdepths[elem]<< endl;
      if (elem.find(target) != std::string::npos && structdepths[elem] == 1 &&
          structBounds[elem].begin <= stoi(celoc)) {
        return elem;
      }
...
public:
  MyASTConsumer(Rewriter &R, PrintingPolicy &pp)
    ...
    VarDepthFinder.addMatcher(varDecl(isExpansionInMainFile()).bind("one"),
                              &varDepthFinder);
    // for decls within labels
    VarDepthFinder.addMatcher(varDecl(hasAncestor(labelStmt().bind("parent")),
                                      isExpansionInMainFile())
                                  .bind("child"),
                              &varDepthFinder);
...
cout << c.first << " " << c.second << endl;
    ...
    */
/*
for(auto l: renamedWhileResolving){
