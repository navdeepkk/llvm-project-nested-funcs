...
public:
  StructBuilder(Rewriter &Rewrite)
      : Rewrite(Rewrite) {} /*
virtual void run(const MatchFinder::MatchResult &Result) {
SourceManager* const sm = Result.SourceManager;
if (const LabelStmt *ls = Result.Nodes.getNodeAs<clang::LabelStmt>("child")){
if (const CompoundStmt *cs =
Result.Nodes.getNodeAs<clang::CompoundStmt>("parent")){ std::string loc =
ls->getBeginLoc().printToString(*sm); loc = loc.substr(loc.find(':') + 1,
loc.find(':')); loc = loc.substr(0,loc.find(':'));
//llvm::errs()<<loc<<"\n";
//iterate through all children and visit all declstmt and then vardecl. get
there type and store in scopes struct
//llvm::errs()<<"label\n";
...
if(const clang::VarDecl *vd = dyn_cast<clang::VarDecl>(*child2)){
   ...
   //llvm::errs()<<varloc<<"\n";
   //if line number of varibale is less than line number of compound stmt add it
to its scope. if(std::stoi(varloc) <= std::stoi(loc)){
...
*/
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    if (const VarDecl *vd = Result.Nodes.getNodeAs<clang::VarDecl>("child")) {
      ...
      for (auto elem : parenChilMap) {
        llvm::errs() << std::stoi(sourceLocs[elem.first]) << " "
                     << std::stoi(sourceLocs[elem.second]) << "\n";
...
public:
  MyASTConsumer(Rewriter &R)
    ...
    Finder.addMatcher(
        ...
        &labelRelBuilder);
    // not removing above logic right now but going to find the variable to
    // build up scope struct. for all nodes find the variables that need to be
    // passed on into its scope.
    ...
    // appropriate redirections for variables may be provided.
  ...
  void HandleTranslationUnit(ASTContext &Context) override {
    ...
    for (auto elem : parenChilMap) {
      llvm::errs() << stoi(sourceLocs[elem.first]) << " "
                   << stoi(sourceLocs[elem.second]) << "\n";
    }
    /*
...
public:
  ...
  /*
    ...
    StringRef file) override { TheRewriter.setSourceMgr(CI.getSourceManager(),
...
stringToDump) : Replace(Replace), stringToDump(stringToDump) {}
...
int main(int argc, const char **argv) {
  ...
  &labelRelBuilder);
  //not removing above logic right now but going to find thte variable to build
  up scope struct.
  ...
  */
  ...
  //The Depths need to be sorted.
    depthSorted = std::vector<std::pair<std::string, int>> (depths.begin(),
  depths.end()); std::sort(depthSorted.begin(), depthSorted.end(), compFunctor);
