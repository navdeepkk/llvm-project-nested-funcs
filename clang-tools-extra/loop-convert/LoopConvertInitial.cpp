...
public :
    virtual void run(const MatchFinder::MatchResult &Result) {
      ...
      if (const LabelStmt *ls =
Result.Nodes.getNodeAs<clang::LabelStmt>("child")){ if (const LabelStmt *lp =
Result.Nodes.getNodeAs<clang::LabelStmt>("parent")){ llvm::errs()<<"Hello"<<
lp->getName(); parenChilMap[ls->getName()] = lp->getName();
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    if (const LabelStmt *ls =
      ...
      if (const CompoundStmt *cs =
              Result.Nodes.getNodeAs<clang::CompoundStmt>("parent")) {
        std::string loc = ls->getBeginLoc().printToString(*sm);
        loc = loc.substr(loc.find(':') + 1, loc.find(':'));
        loc = loc.substr(0, loc.find(':'));
        // llvm::errs()<<loc<<"\n";
        // iterate through all children and visit all declstmt and then vardecl.
        // get there type and store in scopes struct llvm::errs()<<"label\n";
        for (auto child = cs->child_begin(); child != cs->child_end();
             child++) {
          if (const clang::DeclStmt *ds = dyn_cast<clang::DeclStmt>(*child)) {
            for (auto child2 = ds->decl_begin(); child2 != ds->decl_end();
                 child2++) {
              if (const clang::VarDecl *vd =
                      dyn_cast<clang::VarDecl>(*child2)) {
                ...
                // llvm::errs()<<varloc<<"\n";
                // if line number of varibale is less than line number of
                // compound stmt add it to its scope.
                if (std::stoi(varloc) <= std::stoi(loc)) {
                  // llvm::errs()<<ls->getName()<<"\n";
                  // llvm::errs()<<vd->getQualifiedNameAsString()<<"\n";
                  // llvm::errs()<<vd->getType().getAsString()<<"\n";
                  scopes[ls->getName()].name = ls->getName();
                  scopes[ls->getName()].vars[vd->getQualifiedNameAsString()] =
...
public:
  MyASTConsumer(Rewriter &R):  globalBuilder(R), structDumper(R),
labelRelBuilder(R), labelBuilder(R), structBuilder(R){
...
//below matcher returns all node at depth level one.
  Finder.addMatcher(labelStmt(hasParent(compoundStmt(hasParent(functionDecl().bind("parent"))))).bind("child"),
&labelRelBuilder);
//find the parent child relationship of label statements by matching all nodes
having a compound statement as parent;
  Finder.addMatcher(labelStmt(hasParent(compoundStmt(hasParent(labelStmt().bind("parent"))))).bind("child"),
&labelRelBuilder);
//not removing above logic right now but going to find the variable to build up
scope struct.
...
&structBuilder);
//after this call the depths will be sorted. structBuilder has a logicc to sort
the depths.
...
&structDumper);
//Next thing would be to find the depths of the call expressions, so appropriate
redirections for variables may be provided.
...
Compiler.getLangOpts()); return std::unique_ptr<clang::ASTConsumer>(new
MyASTConsumer(this->TheRewriter));
  }

  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef
file) override { TheRewriter.setSourceMgr(CI.getSourceManager(),
CI.getLangOpts()); return make_unique<MyASTConsumer>(TheRewriter);
  }
...
class StructDumper : public MatchFinder::MatchCallback {
public:
  StructDumper(std::map< std::string, Replacements > *Replace, std::string
stringToDump) : Replace(Replace), stringToDump(stringToDump) {} virtual void
run(const MatchFinder::MatchResult &Result) { if (const FunctionDecl *fd =
Result.Nodes.getNodeAs<clang::FunctionDecl>("main")) { Replacement
Rep(*(Result.SourceManager), fd->getLocStart(), 0, stringToDump);
      Replace->insert(Rep);
      }
  }
private:
...
int main(int argc, const char **argv) {
  ...
  //  RefactoringTool refactorTool(OptionsParser.getCompilations(),
  //  OptionsParser.getSourcePathList());
  ...
  Finder.addMatcher(
      labelStmt(hasParent(compoundStmt(hasParent(labelStmt().bind("parent")))))
          .bind("child"),
      &labelRelBuilder);
  // for all nodes find the variables that need to be passed on into its scope.
  Finder.addMatcher(
      labelStmt(hasParent(compoundStmt().bind("parent"))).bind("child"),
      &structBuilder);

  // need to add the nodes immediately nested in main to the parenChilMap.
  // Finder.addMatcher(labelStmt(hasParent(compoundStmt(hasParent(functionDecl(hasName("main")).bind("parent"))))).bind("child"),
  // &labelRelBuilder); not removing above logic right now but going to find
  // thte variable to build up scope struct.
  ...
  //call structDump() to get the structures created.
    ...
    FinderRef.addMatcher(functionDecl(hasName("main")).bind("main"),
  &structDumper); refactorTool.run(newFrontendActionFactory(&FinderRef).get());
